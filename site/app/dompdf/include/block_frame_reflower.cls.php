...
class Block_Frame_Reflower extends Frame_Reflower {
  ...
  protected function _text_align() {
    ...
    case "justify":
      ...
      $comma_width = Font_Metrics::get_text_width(",", $style->font_family, $style->font_size);
      ...
      foreach ($lines as $i => $line) {
        ...
        //if ( $line["left"] + $line["w"] + $line["right"] > self::MIN_JUSTIFY_WIDTH * $width ) {
		  ...
		  $f_text = array();
            ...
            $text = $frame->get_text();
			array_push($f_text, $text);
			 
			if($f_count == 0) {
			}
			else {
				if(substr($f_text[$f_count], 0, 1)!=' ' && substr($f_text[$f_count - 1], -1, 1)!=' ')
				 $count++;
				 else {
				 if($f_text[$f_count]== ' ') $count++;
				 }
			}
			$f_count ++;
          }
          ...
          if($line->wc > 1)
    		    $spacing = $spacing + $_spacing*$count/($line->wc - 1);
          else $spacing = 0;
          ...
          foreach($line->get_frames() as $frame) {
            ...
            $text = $frame->get_text();
            //$frame->get_text() = $text.'<br>';
            $spaces = mb_substr_count($text, " ");
            
			//if(substr($text, -1, 1) !=' ')
			//$spaces=$spaces +1;
			//if(substr($text,0,1) == ',') $extra_space = $comma_width+1;
            ...
            $dx += $spaces * $_spacing + $extra_space;
			$count++;
          }
